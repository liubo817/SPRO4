...
#
# Script generated by Vivado on Mon Mar 30 12:41:18 +0200 2026
...
#
# Usage        : osc.sh
#                osc.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                osc.sh [-rerun]
#                osc.sh [-reset_run]
#                osc.sh [-clean]
#                osc.sh [-reset_log]
...
# script info
echo -e "osc.sh - Script generated by export_simulation (Vivado v2025.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./osc.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  xelab --incr --debug typical --relax --mt 2  -L xil_defaultlib -L secureip --snapshot osc xil_defaultlib.osc -log elaborate.log
}

# RUN_STEP: <simulate>
simulate()
{
  xsim osc -key {Behavioral:sim_1:Functional:osc} -tclbatch cmd.tcl -log simulate.log
}
...
copy_setup_file()
{
  ...
  if [[ ($lib_map_path == "") ]]; then
    lib_map_path="C:/Apps/Vivado/2025.2/Vivado/data/xsim"
  fi
...
reset_run()
{
  files_to_remove=(xelab.pb xsim.jou xvhdl.log xvlog.log compile.log elaborate.log simulate.log xelab.log xsim.log run.log xvhdl.pb xvlog.pb osc.wdb xsim.dir libdpi.so)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./osc.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: osc.sh [-help]\n\
Usage: osc.sh [-step]\n\
Usage: osc.sh [-lib_map_path]\n\
Usage: osc.sh [-rerun]\n\
Usage: osc.sh [-reset_run]\n\
Usage: osc.sh [-clean]\n\
Usage: osc.sh [-reset_log]\n\
Usage: osc.sh [-keep_index]\n\
Usage: osc.sh [-noclean_files]\n\n\
